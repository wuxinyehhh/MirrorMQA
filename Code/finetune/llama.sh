...
if not (parse_ver("4.53.0") <= ver):
    print(f"ERROR: transformers=={transformers.__version__} is unsupported. "
          "Please install: pip install \"transformers>=4.53.0\" -U")
    sys.exit(1)
...
OUTPUT_BASE="$REPO_ROOT/output/Llama"

# Prefer local-disk copy to avoid SIGBUS on some network filesystems. Fall back to shared path if not present.
MODEL_DIR_SHARED="${MODEL_DIR_OVERRIDE:-$REPO_ROOT/models/Llama}"
MODEL_DIR_LOCAL="$REPO_ROOT/models_local/Llama"
...
"$SWIFT_PYTHON" -m torch.distributed.run --nproc_per_node=8 --master_port=29501 \
  ...
  --learning_rate 5e-6 \
  --custom_dataset_info $REPO_ROOT/Dataset/dataset_info_wxy \
  --dataset images_wxy \
  --val_dataset images_wxy_val \
