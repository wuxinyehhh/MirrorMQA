...
MODEL = "gemini-3-flash-preview"   # 必须是支持图像输入的模型（按你的代理实际可用模型修改）
MAX_WORKERS = 3
DATA_DIR = os.path.join(os.path.dirname(__file__), "image")
OUTPUT_FILE = os.path.join(DATA_DIR, "predictions_gemini_1.tsv")
...
def clean_tsv():
    """删除 predictions_gemini_1.tsv 中图片文件在 DATA_DIR 下实际不存在的行。"""
